...
#[allow(unused_imports)]
use qkd014_server_gen::{Api, ApiNoContext, Claims, Client, ContextWrapperExt, models,
                      GetKeyResponse,
...
fn main() {
    ...
    match matches.get_one::<String>("operation").map(String::as_str) {
        Some("GetKey") => {
            let result = rt.block_on(client.get_key(
                  "slave_sae_id_example".to_string(),
                  None
            ));
            info!("{:?} (X-Span-ID: {:?})", result, (client.context() as &dyn Has<XSpanIdString>).get().clone());
        },
        Some("GetKeySimple") => {
            let result = rt.block_on(client.get_key_simple(
                  "slave_sae_id_example".to_string(),
                  Some(56),
                  Some(56)
            ));
        ...
        Some("GetKeyWithIds") => {
            let result = rt.block_on(client.get_key_with_ids(
                  "master_sae_id_example".to_string(),
                  ???
            ));
            info!("{:?} (X-Span-ID: {:?})", result, (client.context() as &dyn Has<XSpanIdString>).get().clone());
        },
        */
        Some("GetKeyWithIdsSimple") => {
            let result = rt.block_on(client.get_key_with_ids_simple(
                  "master_sae_id_example".to_string(),
        ...
        Some("GetStatus") => {
            let result = rt.block_on(client.get_status(
                  "slave_sae_id_example".to_string()
            ));
            info!("{:?} (X-Span-ID: {:?})", result, (client.context() as &dyn Has<XSpanIdString>).get().clone());
        },
